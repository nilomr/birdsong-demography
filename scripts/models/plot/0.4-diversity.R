...
div_m_3 <- readRDS(file.path(config$path$fits, "div_m_3.rds"))
div_m_3.1 <- readRDS(file.path(config$path$fits, "div_m_3.1.rds"))
...
div_m_12_margeffs_mefs <- dplyr::bind_rows(
...
div_m_3_mefs <- dplyr::bind_rows(
    calc_avg_slopes(div_m_3, "diversity"),
    calc_avg_slopes(div_m_3.1, "diversity")
)
...
div_m_1_gr <- marginaleffects::datagrid(
    model = div_m_1,
    mean_dispersal_distance = div_m_1$data$mean_dispersal_distance
)
...
div_m_1_pred <- marginaleffects::predictions(div_m_1,
    ...
    newdata = marginaleffects::datagrid(
        mean_dispersal_distance = seq(
            min(div_m_1$data$mean_dispersal_distance),
            max(div_m_1$data$mean_dispersal_distance),
            by = 0.1
        )
    )
) |>
...
div_m_1_gr_2 <- og_scale(div_data, div_m_1_gr_2, v = "mean_dispersal_distance")
div_m_1_pred <- og_scale(div_data, div_m_1_pred, v = "mean_dispersal_distance")



# Effect of age on cultural diversity (div_m_2)

div_m_2_gr <- marginaleffects::datagrid(
    model = div_m_2,
    mean_age = div_m_2$data$mean_age
)

div_m_2_gr_2 <- partial_residuals(div_m_2, div_m_2_gr)
div_m_2_pred <- marginaleffects::predictions(div_m_2,
    type = "response",
    re_formula = NULL,
    ndraws = 500,
    newdata = marginaleffects::datagrid(
        mean_age = seq(
            min(div_m_2$data$mean_age),
            max(div_m_2$data$mean_age),
            by = 0.1
        )
    )
) |>
...
# convert estimates back to the original scale
div_m_2_gr_2 <- og_scale(div_data, div_m_2_gr_2, v = "mean_age")
div_m_2_pred <- og_scale(div_data, div_m_2_pred, v = "mean_age")
...
div_m_12_margeffs <- div_m_12_margeffs_mefs |>
...
div_m_2_spp <- get_spatial_preds(div_m_2, div_m_2$data, resolution, ndraws)
div_m_2_spp <- div_m_2_spp |> # group by x and y and get mean of estimate and se
    dplyr::group_by(x, y) |>
    dplyr::summarise(
...
div_m_2_rast <- get_raster(div_m_2_spp, pop_contour, NULL, type, resolution,
    fact = 4
)
crange <- div_m_2_rast |>
    dplyr::select(!!type) |>
...
div_m_2_spat <- ggplot() +
    geom_raster(data = div_m_2_rast, aes(x = x, y = y, fill = !!sym(type))) +
    geom_sf(
    ...
    scale_x_continuous(
        expand = c(0, 0),
        limits = c(min(div_m_2_rast$x) - 200, max(div_m_2_rast$x) + 800),
    ) +
    theme(
        aspect.ratio = NULL,
...
# join plots
diversity_all <- div_m_12_margeffs + div_m_1_disp + div_m_2_age + div_m_2_spat +
    plot_layout(ncol = 4) +
