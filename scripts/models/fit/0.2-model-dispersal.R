...
# brms::pp_check(disp_m_1)
# brms::pp_check(disp_m_1, type = "stat_2d")
# brms::stancode(disp_m_1)
# plot(disp_m_1)
...
disp_m_1_ppcheck <- brms::pp_check(disp_m_1, type = "stat_2d", ndraws = 1000) +
    titheme() +
    labs(
        y = "SD", x = "Mean",
        title = "Posterior predictive check",
        subtitle = "Model: disp_m_1"
    ) +
    theme(
...
ggsave(
    file.path(config$path$figures, "disp_m_1_ppcheck.png"),
    plot = disp_m_1_ppcheck,
    width = 10,
    height = 10,
    units = "cm",
    dpi = 300,
...
disp_m_1_draws1 <- marginaleffects::avg_slopes(disp_m_1,
    ...
    newdata = marginaleffects::datagrid(
        nest_distance = min(disp_m_1_data_std$nest_distance)
    )
) |>
...
disp_m_1_draws2 <- marginaleffects::avg_slopes(disp_m_1,
    ...
    re_formula = NULL,
    newdata = marginaleffects::datagrid(
        natal_distance = min(disp_m_1_data_std$natal_distance)
    )
) |>
    marginaleffects::posterior_draws() |>
    dplyr::as_tibble() |>
...
# save to file.path(config$path$figures, "individual_dispersal.rds")
saveRDS(fullplot, file.path(config$path$figures, "individual_dispersal.rds"))

ggsave(
    file.path(config$path$figures, "individual_dispersal.png"),
    plot = fullplot,
    width = 15,
    height = 10,
    units = "cm",
...
disp_m_1grid <- marginaleffects::datagrid(
    model = disp_m_1,
    nest_distance = disp_m_1_data_std$nest_distance,
    natal_distance = c(
        max(disp_m_1_data_std$natal_distance),
        mean(disp_m_1_data_std$natal_distance),
        min(disp_m_1_data_std$natal_distance)
    ),
    year_born_diff = "1",
    year = "2021"
)
...
data <- disp_m_1_data_std
e <- residuals(disp_m_1, summary = TRUE)[, "Estimate"]
grid <- match_grid(disp_m_1grid, data)

disp_m_1_grid2 <- marginaleffects::predictions(disp_m_1,
    newdata = grid,
    type = "response", re_formula = NULL
)
disp_m_1_grid2[["type"]] <- NULL
disp_m_1_grid2[["estimate_with_error"]] <- disp_m_1_grid2[["estimate"]] + e
disp_m_1_grid2 <- dplyr::as_tibble(disp_m_1_grid2)

disp_m_1_grid2 <- og_scale(disp_m_1_data, disp_m_1_grid2, v = "nest_distance")
disp_m_1_grid2 <- og_scale(disp_m_1_data, disp_m_1_grid2, v = "natal_distance")

# round natal_distance to the nearest integer
disp_m_1_grid2 <- disp_m_1_grid2 |>
...
disp_m_1_p1 <- disp_m_1_grid2 |>
    ...
    scale_fill_manual(
        values = f_colors,
        labels = paste0(unique(disp_m_1_grid2$natal_distance), "m")
    ) +
    guides(colour = "none") +
...
# Plot predictions instead?
mfx <- marginaleffects::predictions(disp_m_1,
    type = "response",
    re_formula = NULL,
    newdata = marginaleffects::datagrid(
        nest_distance = c(min(disp_m_1_data_std$nest_distance), mean(disp_m_1_data_std$nest_distance)),
        natal_distance = c(min(disp_m_1_data_std$natal_distance), mean(disp_m_1_data_std$natal_distance)),
        year = 2021,
        year_born_diff = "1"
    )
) |>
...
ggplot(mfx, aes(x = draw, fill = factor(nest_distance), color = factor(natal_distance))) +
    ggdist::stat_halfeye(slab_alpha = .5) +
    labs(
        x = "Marginal Effect of Age on Survival",
        y = "Posterior density"
    )
