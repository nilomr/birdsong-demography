...
div_data_std <- div_data |>
    dplyr::mutate_at(
        vars(
            mean_age, neighbours, prop_same_birds, recorded,
...
nov_m_1 <- brms::brm(
    ...
    file = file.path(config$path$fits, "nov_m_1"),
    file_refit = "never",
)
...
nov_m_2 <- brms::brm(
    ...
    threads = brms::threading(2),
    backend = "cmdstanr",
    file = file.path(config$path$fits, "nov_m_2"),
    file_refit = "never",
)
...
nov_m_2.1 <- brms::brm(
    ...
    threads = brms::threading(2),
    backend = "cmdstanr",
    file = file.path(config$path$fits, "nov_m_2.1"),
    file_refit = "never",
)

# ──── ABSOLUTE DIVERSITY ─────────────────────────────────────────────────────

# Does a greater proportion of immigrant birds lead to more absolute diversity?


# prepare data

div_data_m2_std <- div_data |>
    dplyr::mutate_at(
        vars(
            mean_age, neighbours, prop_same_birds, recorded,
            prop_immigrant, mean_dispersal_distance
...
div_f_2 <- brms::bf(
    n_unique_current_songs ~ 0 + prop_immigrant + mean_dispersal_distance +
        prop_same_birds + mean_age + s(recorded) + year +
        gp(x, y, by = year, k = 25, c = 5 / 4, scale = FALSE)
)

div_m_2 <- brms::brm(
    div_f_2,
    data = div_data_m2_std,
    family = gaussian(),
    iter = 1500,
    warmup = 500,
    chains = 4,
...
# Compare with model adjusting for number of songs recorded, not number of birds

div_f_2.1 <- brms::bf(
    n_unique_current_songs ~ 0 + prop_immigrant + mean_dispersal_distance +
        prop_same_birds + mean_age + s(n_current_songs) + year +
        gp(x, y, by = year, k = 25, c = 5 / 4, scale = FALSE)
)
...
div_m_2.1 <- brms::brm(
    ...
    threads = brms::threading(2),
    backend = "cmdstanr",
    file = file.path(config$path$fits, "div_m_2.1"),
    file_refit = "never",
)
