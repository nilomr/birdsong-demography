...
length(unique(manual_labels$class_label))
...
neigh_data <- main_data |>
    ...
    dplyr::ungroup() |>
    dplyr::select(
        pnum, pnum2, nestbox, year, year2, neighbours, recorded, father, father_list,
...
shframe <-
    ...
    dplyr::filter(pnum != pnum2, nest_distance < n_radius) |>
...
sharing_df <- dplyr::left_join(
    ...
    dplyr::mutate(
        ...
        n_shared = unlist(purrr::map2(current_labels, past_labels, ~ length(intersect(.x, .y)))),
...
# plot the prop_shared vs prop_same_birds
neigh_data |>
    ggplot(aes(x = mean_age, y = prop_shared, size = neighbours)) +
    geom_point() +
...
# Need: dispersal distance vs n unique songs and rare songs
#       age vs rate of change FOR NEIGHBOURHOODS, AND CALCULATE FOR THE ENTIRE POP


# DEFINE AND FIT nm1 ─────────────────────────────────────────────────────── #

nm1 <- brms::bf(
    prop ~ 1 + resident_status * year_born_diff * nest_distance +
