...
nm_data_std |>
    # divide the neighbours variable into two groups, splitting on the mean
    dplyr::mutate(
        n_songs_current = ifelse(
            n_songs_current > mean(n_songs_current),
            "high",
            "low"
        )
    ) |>
    ggplot(aes(
        x = prop_resident, y = prop_rare,
        fill = n_songs_current,
        color = n_songs_current
    )) +
    geom_point() +
